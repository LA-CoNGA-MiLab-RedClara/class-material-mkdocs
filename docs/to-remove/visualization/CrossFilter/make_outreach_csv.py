...
def get_data_from_files():
    ...
    df.to_csv('13TeV_outreach.csv',index=False)
    for s in samples:
...
def read_file(path,sample):
    ...
    for data in mc.iterate(["lep_n","lep_pt","lep_eta","lep_phi","lep_E","lep_charge","lep_type",
                         ...
                         "mcWeight","scaleFactor_PILEUP","scaleFactor_ELE","scaleFactor_MUON", # add more variables here if you make cuts on them ,              
                            "scaleFactor_LepTRIGGER"], flatten=False, entrysteps=2910540, outputtype=pd.DataFrame, entrystart=entrystart, entrystop=numevents*fraction_MC):
